...
@SpringBootTest
@ActiveProfiles("test")
class SpringKafkaWorkaroundTests {
    ...
    static class RetryableKafkaListeners {
        ...
        @KafkaListener(topics = SpringKafkaWorkaroundTests.FIRST_TOPIC)
        void firstListener(String message) {
        }

        @RetryableTopic(kafkaTemplate = "kafkaTemplate")
        @KafkaListener(topics = SpringKafkaWorkaroundTests.SECOND_TOPIC)
